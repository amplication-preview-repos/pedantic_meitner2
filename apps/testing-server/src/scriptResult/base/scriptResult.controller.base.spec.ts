...
const service = {
  ...
  scriptResults: () => FIND_MANY_RESULT,
  scriptResult: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("ScriptResult", () => {
  ...
  test("GET /scriptResults/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/scriptResults"}/${nonExistingId}`)
  ...
  test("GET /scriptResults/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/scriptResults"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /scriptResults existing resource", async () => {
